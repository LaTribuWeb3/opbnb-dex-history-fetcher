  ...
  while (true) {
    ...
    try {
      ...
      for (const span of SPANS) {
        ...
        const txResponse = await retry(pythiaContract.multiSet, [
          allAssets,
          allKeys,
          allValues,
          allUpdateTimes,
  ...
  for (const platform of PLATFORMS_TO_USE) {
    ...
    if (liquidityAverage.avgPrice != 0) {
      ...
      for (const slippagePct of TARGET_SLIPPAGES) {
        ...
        const avgVolumeForSlippageValidDecimals = new BigNumber(avgVolumeForSlippage)
          .times(BigNumber(10).pow(tokenConf.decimals))
          .toFixed(0);
        results.push({
          asset: tokenConf.address,
          key: key,
...
function encodeVolatilityKey(collateralAsset, debtAsset, mode, period) {
  return ethers.utils.keccak256(
    ethers.utils.defaultAbiCoder.encode(
      ...
      ['volatility', debtAsset, mode, period]
    )
  );
}
...
function encodeLiquidityKey(collateralAsset, debtAsset, source, slippage, period) {
  return ethers.utils.keccak256(
    ethers.utils.defaultAbiCoder.encode(
      ...
      ['liquidity', debtAsset, source, slippage, period]
    )
  );
}
