...
function v2_computeLiquidityForSlippagepancakePoolCryptoV2(
  baseAmountPrice,
  baseQty,
  targetPrice,
  baseReserves,
...
// this is the old function: avg slippage
function computeLiquidityForSlippagepancakePoolCryptoV2(
  baseQty,
  targetPrice,
  reserves,
  i,
...
function computePriceAndSlippageMapForReserveValue(fromSymbol, toSymbol, poolTokens, ampFactor, reserves) {
  ...
  for (let slippageBps = 50; slippageBps <= 2000; slippageBps += 50) {
    ...
    const liquidityObj = v2_computeLiquidityForSlippagepancakePool(
      lastAmount,
      targetPrice,
      reservesNorm18Dec,
      indexFrom,
...
function computePriceAndSlippageMapForReserveValueCryptoV2(
  ...
  for (let slippageBps = 50; slippageBps <= 2000; slippageBps += 50) {
    const targetPrice = price - (price * slippageBps) / 10000;
    const liquidityObj = v2_computeLiquidityForSlippagepancakePoolCryptoV2(
      baseAmount,
      lastAmount,
      targetPrice,
      reserves,
...
module.exports = {
  getAvailablepancake,
  getpancakeDataforBlockInterval,
