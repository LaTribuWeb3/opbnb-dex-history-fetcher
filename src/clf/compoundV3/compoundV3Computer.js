...
async function compoundV3Computer(fetchEveryMinutes, startDate = Date.now()) {
  ...
  try {
    ...
    for (const pool of Object.values(compoundV3Pools)) {
      results[pool.baseAsset] = await computeCLFForPool(
        pool.cometAddress,
        pool.baseAsset,
        Object.values(pool.collateralTokens),
        web3Provider,
        fromBlocks,
        currentBlock,
        startDateUnixSecond
 ...
 */
async function computeCLFForPool(
  cometAddress,
  baseAsset,
  collaterals,
  web3Provider,
  ...
  for (const collateral of collaterals) {
    try {
      ...
      resultsData.collateralsData[collateral.symbol].collateral = await getCollateralAmount(
        collateral,
        cometContract,
        startDateUnixSec,
        endBlock
      ...
      resultsData.collateralsData[collateral.symbol].clfs = await computeMarketCLFBiggestDailyChange(
        assetParameters,
        collateral,
        baseAsset,
        fromBlocks,
    ...
    if (blockNumberForSpan.length > 0) {
      ...
      for (const blockNumber of blockNumberForSpan) {
        sumLiquidityForTargetSlippageBps +=
          fullLiquidity[blockNumber].slippageMap[assetParameters.liquidationBonusBPS].base;
      }
  ...
  for (const volatilitySpan of spans) {
    ...
    for (const liquiditySpan of spans) {
      results[volatilitySpan][liquiditySpan] = findRiskLevelFromParameters(
        ...
        assetParameters.LTV,
        (assetParameters.supplyCap * assetParameters.LTV) / 100
      );
    }
  }
