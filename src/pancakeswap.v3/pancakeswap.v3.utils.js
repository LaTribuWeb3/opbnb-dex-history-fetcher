...
function getSlippages(currentTick, tickSpacing, sqrtPriceX96, liquidity, token0Decimals, token1Decimals) {
  const token0Slippage = GetAmountXDumpable(
    currentTick,
    tickSpacing,
    liquidity,
    token0Decimals,
  ...
  const token1Slippage = GetAmountYDumpable(
    currentTick,
    tickSpacing,
    liquidity,
    token0Decimals,
...
function getUniV3DataContents(selectedFiles, dataDir, minBlock = 0) {
  ...
  for (let i = 0; i < selectedFiles.length; i++) {
    ...
    const fileContent = fs
      .readFileSync(path.join(dataDir, 'pancakeswapv3', selectedFile), 'utf-8')
      .split('\n')
...
module.exports = {
  getPriceNormalized,
  getSlippages,
  getAvailablepancakeswapV3,
  getUniV3DataforBlockInterval,
  getPriceFromSqrt,
  getAllPoolsToFetch
};
