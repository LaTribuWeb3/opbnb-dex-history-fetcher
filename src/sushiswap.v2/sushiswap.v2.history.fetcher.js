...
async function SushiswapV2HistoryFetcher(onlyOnce = false) {
  ...
  while (true) {
    ...
    try {
      ...
      fs.writeFileSync(
        path.join(DATA_DIR, 'sushiswapv2', 'sushiswapv2-fetcher-result.json'),
...
async function FetchHistoryForPair(web3Provider, pairConfig, currentBlock, minStartBlock) {
  ...
  if (!fs.existsSync(historyFileName)) {
    fs.writeFileSync(
      historyFileName,
      `blocknumber,reserve_${baseConf.symbol}_${baseConf.address},reserve_${quoteConf.symbol}_${quoteConf.address}\n`
    );
  } else {
  ...
  console.log(
    `${fnName()}[${pairConfig.base}/${pairConfig.quote}]: start fetching data for ${
  ...
  while (toBlock < currentBlock) {
    ...
    console.log(
      `${fnName()}[${pairConfig.base}/${pairConfig.quote}]: [${fromBlock} - ${toBlock}] found ${
        events.length
