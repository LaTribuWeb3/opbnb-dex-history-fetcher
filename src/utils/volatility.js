...
function computeBiggestDailyChange(medianPricesAtBlock, currentBlock) {
  ...
  while (currBlock <= currentBlock) {
    ...
    const medianPricesForDay = medianPricesAtBlock
      .filter((_) => _.block >= currBlock && _.block < stepTargetBlock)
      .map((_) => _.price);
    if (medianPricesForDay.length > 0) {
      ...
      if (priceChangePctForDay > biggestPriceChangePct) {
        label = `Biggest price change on day ${cptDay} for interval [${currBlock}-${stepTargetBlock}]: ${roundTo(
          priceChangePctForDay * 100
        )}%. [${minPriceForDay} <> ${maxPriceForDay}]`;
...
module.exports = {
  ...
  computeBiggestDailyChange,
  medianPricesOverBlocks,
  rollingBiggestDailyChange
};
